...
class SvnWikiView(object):
    ...
    def new_file_default_mimetype_policy(self, request):
    ...
    def get_response(self, template, data, content_type=None):
        ...
        data['mimetype'] = content_type = data.get('kind') or data.get('mimetype') or 'text/html'
    ...
    def post(self, request):
        ...
        if request.GET.has_key('factory'):
            ...
            self.svn(request).write(uri, contents, message, mimetype)
            return exc.HTTPSeeOther(location=request.path_info)

        try:
            self.svn(request).write(request.path_info, contents, message, mimetype)
        except NotAFile:
            if mimetype:
                self.svn(request).set_mimetype(request.path_info, mimetype, message)
            else:
                import random
                title = random.randint(0, 1000000)
                uri = '/'.join((request.path_info, str(title)))
    ...
    def edit_view(self, request):
        try:
            ...
            contents = {
                'body': '',
                'mimetype': self.new_file_default_mimetype_policy(request),
                }        
            return self.get_response('edit.html', contents, "text/html")

        contents['mimetype'] = self.svn(request).mimetype(request.path_info)
    ...
    def default_view(self, request):
        ...
        return self.get_response('view.html', contents, contents['kind'] or 'text/html')
